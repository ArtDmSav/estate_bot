...
from sqlite_message_db import write_user, stop_user
from tg_keyboards import choose_city_button, keyboard_max_price_buttons, keyboard_min_price_buttons, restart_button

logging.basicConfig(level=logging.DEBUG, filename='bot_estate.log',
...
async def city(message: types.Message, state: FSMContext):
    ...
    await message.reply('Укажите минимальную стоимость аренды:  \n',
                        reply_markup=keyboard_min_price_buttons
                        )
    print(data['gender'])
...
async def min_price(message: types.Message, state: FSMContext):
    ...
    await message.reply('Укажите максимальную стоимость аренды:  \n',
                        reply_markup=keyboard_max_price_buttons
                        )
...
async def max_price(message: types.Message, state: FSMContext):
    ...
    if int(data['min_price']) <= int(data['max_price']):
        write_user(data['city'], int(data['min_price']), int(data['max_price']), message.chat.id,
                   1, datetime.now())  # datatime change to datatime last message
    else:
